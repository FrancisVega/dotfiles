...
squash_merge() {
  if [ -z "$1" ]; then
    echo "Uso: sm <rama_origen>"
    return 1
  fi

  rama_origen="$1"

  # Obtiene la rama actual (main)
  rama_destino=$(git branch --show-current)

  # Realiza el squash merge
  git merge --squash --ff "$rama_origen"

  # Crea el commit con el mensaje predeterminado
  git commit -m "Merge $rama_origen into $rama_destino"

  echo "Squash merge de $rama_origen en $rama_destino completado."
}
...
alias ",gb"='git switch $(git branch | fzf --exact)'

# Squash merge
alias ",sm"='squash_merge'
